...
p_project = "Dodow PWM - Funccion Respiracion lineal en 500 pasos- Gamma correcion"
p_version = "2.0"
...
def respiraLGPas(pinpwm , duracionrespms, debug = False):
    """ Realiza una respiracion = subir luz led + bajar led - curvas lineales 500 pasos- corr Gamma
    ...
    for p in range(PASOS):
        # correccion  gamma
        pinpwm.duty_u16(MINPWM + int(gamma((p*PASOPWM)/RANPWM)*RANPWM))
    ...
    for p in range(PASOS):
        # correcion gamma
        pinpwm.duty_u16(MAXPWM - int(gamma((p*PASOPWM)/RANPWM)*RANPWM))
    ...
    return 'Respiracion Lineal 4_5_1 - 500 pasos- Gamma correcion'
...
if (__name__ == '__main__'):
    ...
    print('Tipo =',respiraLGPas(pwmLed, durarepsactualms, DEBUG))
